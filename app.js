...
/* ═══════════════════════════════════════════════════════════
   DEMO DATA — solo se carga cuando el usuario lo pide
   explícitamente desde Config → "Cargar datos demo".
   NUNCA se carga automáticamente al abrir el sistema.
═══════════════════════════════════════════════════════════ */
function loadDemoData(force = false) {
  // Solo ejecutar si el usuario presionó el botón explícitamente
  if (!force) return;

  if (!confirm('¿Cargar datos demo? Esto reemplazará todos los datos actuales.')) return;

  // Limpiar todo antes de cargar demo
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initNav();
  migrateData();
  // NO se llama loadDemoData() aquí.
  // El sistema arranca vacío en cualquier navegador nuevo.
  // Los datos demo se cargan solo desde Config → "Cargar datos demo".
