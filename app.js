...
function loadDemoData(force = false) {
  // ── CORRECCIÓN DE BUG ────────────────────────────────
  // Antes se chequeaba load('equipos').length > 0, lo que causaba
  // que al borrar TODOS los equipos volvieran a cargarse.
  // Ahora se usa una flag 'cit_initialized' en localStorage.
  // Si la flag existe, el sistema ya fue inicializado y no se
  // tocan los datos del usuario aunque haya borrado todo.
  const INIT_KEY = 'cit_initialized';
  if (!force && localStorage.getItem(INIT_KEY)) return;

  // Marcar como inicializado ANTES de cargar datos
  localStorage.setItem(INIT_KEY, '1');
  // Al forzar recarga (botón "Cargar datos demo"), limpiar todo primero
  if (force) Object.keys(KEYS).forEach(k => save(k, []));
...
function clearDemoData() {
  ...
  localStorage.setItem(DATA_VERSION_KEY, DATA_VERSION);
  // Conservar la flag de inicializado para que los datos no vuelvan solos
  // (el usuario eligió explícitamente limpiar)
  localStorage.setItem('cit_initialized', '1');
  renderDashboard();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── PASO 1: Proteger datos existentes ─────────────────
  // Antes de CUALQUIER otra cosa, marcar la flag si ya hay
  // datos guardados. Esto cubre usuarios que venían de la
  // versión anterior donde la flag no existía.
  // La condición chequea la CLAVE en sí, no el contenido —
  // así cubre el caso donde el usuario borró todos los items
  // (array vacío [] sigue siendo datos válidos del usuario).
  const claveEquipos = localStorage.getItem(KEYS.equipos);
  if (claveEquipos !== null) {
    // La clave existe → el sistema ya fue inicializado antes
    localStorage.setItem(INIT_KEY, '1');
  }
  ...
  initNav();
  migrateData();
  loadDemoData();   // no hace nada si INIT_KEY existe
